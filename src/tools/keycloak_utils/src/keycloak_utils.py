...
class KeycloakServiceAccount(object):
    ...
    def is_token_valid(self,access_token):
        ...
        if self.debug:
            logging.info("Checks if access token is valid")     
        ...
        try:
            ...
            if self.debug:
                logging.info(f"Response from {url}: {resp.json()}")
    ...
    def get_kc_tokens_from_refresh(self,refresh_token):
        ...
        logging.info("Getting new access token from refresh token")
    ...
    def service_account_login(self,func):
        ...
        def wrapper_service_account_login(*args, **kwargs):
            ...
            if self.TOKENS["access_token"] != None:
                if self.debug:
                    logging.info("Session was initiated at least once")
                ...
                if self.debug:
                    logging.info("Access token not valid")
